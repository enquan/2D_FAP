...
from MobileNetv2_modified import *
from ResNet import *
from VGG import *
from AttNet import *
...
class SCUT(Dataset):
    ...
    def read_img(self, img_dir, datafile):
        ...
        for line in lines:
            ...
            # level = [float(linesplit[3]), float(linesplit[4]), float(linesplit[5]),float(linesplit[6]),float(linesplit[7])]
            ...
            # if self.dataset == 5500:
            #     target = [cdf(x[i+1],score,std)-cdf(x[i],score,std) for i in range(40)]
            #     # target = [sampling(x, score, std) for x in np.arange(1,5.1,0.1)]
            # elif self.dataset == 500:
            #     target = [cdf(x[i+1],score,std)-cdf(x[i],score,std) for i in range(40)]
            #     # target = [sampling(x, score, std) for x in np.arange(1,5.1,0.1)]
...
if args.dataset == 5500:
    if args.aligned == True:
        if args.MTCNN == False:
            ...
            img_dir = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/aligned'
            trainfile = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/train_test_files/5_folders_cross_validations_files/cross_validation_'+str(fold)+'/train_'+str(fold)+'_aligned_ex.txt'
            testfile = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/train_test_files/5_folders_cross_validations_files/cross_validation_'+str(fold)+'/test_'+str(fold)+'_aligned_ex.txt'
            ...
            img_dir = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/MTCNN'
            trainfile = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/train_test_files/5_folders_cross_validations_files/cross_validation_'+str(fold)+'/train_'+str(fold)+'_mtcnn.txt'
            testfile = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/train_test_files/5_folders_cross_validations_files/cross_validation_'+str(fold)+'/test_'+str(fold)+'_mtcnn.txt'
        ...
        img_dir = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/Images'
        trainfile = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/train_test_files/5_folders_cross_validations_files/cross_validation_'+str(fold)+'/train_'+str(fold)+'.txt'
        testfile = '/public/home/hpc204712173/SCUT-FBP5500/SCUT-FBP5500_v2/train_test_files/5_folders_cross_validations_files/cross_validation_'+str(fold)+'/test_'+str(fold)+'.txt'
        train_data = SCUT(dataset=args.dataset, img_dir=img_dir, datafile=trainfile, transform=transform['train'])
        test_data = SCUT(dataset=args.dataset, img_dir=img_dir, datafile=testfile, transform=transform['test'])
    if args.network == 'vgg16' or args.network == 'vgg19':
    ...
    #########################
    # test_split = 0.2
    # img_dir = '/public/home/hpc204712173/SCUT-FBP/Images'
    # dataset = '/public/home/hpc204712173/SCUT-FBP/score.txt'
    # full_data = SCUT(dataset=args.dataset, img_dir=img_dir, datafile=dataset, transform=transform)
    ...
    # # Ref: https://www.cnblogs.com/marsggbo/p/10496696.html

    # train_loader = DataLoader(dataset=full_data, batch_size=batch_size, sampler=train_sampler, num_workers=8, pin_memory=True)
    # test_loader = DataLoader(dataset=full_data, batch_size=batch_size, sampler=test_sampler, num_workers=8, pin_memory=True)
    # warnings.filterwarnings('ignore')
    ##########################
    # train_img_dir = '/public/home/hpc204712173/SCUT-FBP/Liu/TrainImagesWithAugmentation'
    # test_img_dir = '/public/home/hpc204712173/SCUT-FBP/Liu/TestImages'
    # trainfile = '/public/home/hpc204712173/SCUT-FBP/train_full.txt'
    # testfile = '/public/home/hpc204712173/SCUT-FBP/test.txt'
    train_img_dir = '/public/home/hpc204712173/SCUT-FBP/MTCNN/train'
    test_img_dir = '/public/home/hpc204712173/SCUT-FBP/MTCNN/test'
    trainfile = '/public/home/hpc204712173/SCUT-FBP/train_2.txt'
    testfile = '/public/home/hpc204712173/SCUT-FBP/test_2.txt'
    ...
    img_dir = '/public/home/hpc204712173/3dface/BJUT-3D_texture'
    trainfile = '/public/home/hpc204712173/3dface/train_liu_full_texture.txt'
    testfile = '/public/home/hpc204712173/3dface/test_liu_full_texture.txt'
    train_data = SCUT(dataset=args.dataset, img_dir=img_dir, datafile=trainfile, transform=transform['train'])
    test_data = SCUT(dataset=args.dataset, img_dir=img_dir, datafile=testfile, transform=transform['test'])
    train_loader = DataLoader(dataset=train_data, batch_size=batch_size, num_workers=8, pin_memory=True, shuffle=True)
    test_loader = DataLoader(dataset=test_data, batch_size=batch_size, num_workers=8, pin_memory=True, shuffle=False)
...
if args.network == 'mobilenet_m':
    ...
    pretrained_model = torch.load('/public/home/hpc204712173/.cache/torch/hub/checkpoints/mobilenet_v2-b0353104.pth')
...
if args.network == 'vgg16':
    ...
    pretrained_model = torch.load('/public/home/hpc204712173/.cache/torch/hub/checkpoints/vgg16_bn-6c64b313.pth')
    model_dict = net.state_dict()
    state_dict = {k:v for k, v in pretrained_model.items() if (k in model_dict.keys() and v.size()==model_dict[k].size())}
    model_dict.update(state_dict)
    net.load_state_dict(model_dict)
...
if args.network == 'vgg19':
    ...
    pretrained_model = torch.load('/public/home/hpc204712173/.cache/torch/hub/checkpoints/vgg19_bn-c79401a0.pth')
    model_dict = net.state_dict()
    state_dict = {k:v for k, v in pretrained_model.items() if (k in model_dict.keys() and v.size()==model_dict[k].size())}
    model_dict.update(state_dict)
    net.load_state_dict(model_dict)
...
# pretrained_model = torch.load('/public/home/hpc204712173/.cache/torch/hub/checkpoints/mobilenet_v2-b0353104.pth')
# pretrained_model = torch.load('/public/home/hpc204712173/mobilenetv2_Top1_71.806_Top2_90.410.pth.tar')
...
def test():
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader):
            ...
            # plevel = torch.zeros(output.shape[0], 5, dtype=torch.float).to(device)
            # cnt = torch.tensor([5, 10, 10, 10, 5]).to(device)
            # idx = [i.repeat(times) for i, times in zip(torch.arange(len(cnt)),cnt)]
            # idx = torch.cat(idx).to(device)
            # plevel.index_add_(dim=1, index=idx, source=output)

            # pred_score = torch.sum(plevel*rank, dim=1)  # (n)
...
def train(epoch, rank, alpha, beta, gamma):
    ...
    for i, inputs in enumerate(train_loader):
        ...
        # plevel = torch.zeros(outputs.shape[0], 5, dtype=torch.float).to(device)
        # cnt = torch.tensor([5, 10, 10, 10, 5]).to(device)
        ...
        cnt[0] -= 5
        cnt[-1] -= 5
        # cnt = torch.tensor([5, 10, 10, 10, 10, 10, 10, 10, 10, 5]).to(device)
        idx = [i.repeat(times) for i, times in zip(torch.arange(len(cnt)),cnt)]
        idx = torch.cat(idx).to(device)
        ...
        # weight = torch.tensor([159328/19794, 159328/67634, 159328/159328, 159328/62272, 159328/20972],dtype=torch.float).to(device)
        # weight = torch.tensor([2721/28, 2721/1318, 2721/2721, 2721/1395, 2721/38],dtype=torch.float).to(device)
...
# rank = torch.Tensor([i for i in range(41)]).to(device)
# rank = torch.Tensor([i for i in np.arange(1,5.1,0.1)]).to(device)
# rank = torch.Tensor([i for i in np.arange(1.05,5.05,0.1)]).to(device)
...
# rank = torch.Tensor([i for i in range(1,6)]).to(device)
