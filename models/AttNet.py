...
class TinyAttNet(nn.Module):  # compression rate = 0.25
    def __init__(self):
        super(TinyAttNet, self).__init__()
...
class ThinAttNet(nn.Module):  # compression rate = 0.5
    def __init__(self):
        super(ThinAttNet, self).__init__()
...
class ThinAttNet_m(nn.Module):
    def __init__(self):
        super(ThinAttNet_m, self).__init__()
        self.conv1 = Conv(3, 64, 3, 1, 1)# 64
        self.conv2 = Conv(64, 64, 3, 1, 1)
        self.pool1 = nn.MaxPool2d(2, 2)
        self.conv3 = Conv(64, 128, 3, 1, 1)# 128
        self.conv4 = Conv(128, 128, 3, 1, 1)
        self.pool2 = nn.MaxPool2d(2, 2)
        self.conv5 = Conv(128, 256, 3, 1, 1)# 256
        self.conv6 = Conv(256, 256, 3, 1, 1)
        self.conv7 = Conv(256, 256, 3, 1, 1)
        self.pool3 = nn.MaxPool2d(2, 2)
        self.conv8 = Conv(256, 512, 3, 1, 1)# 512
        self.conv9 = Conv(512, 512, 3, 1, 1)
        self.conv10 = Conv(512, 512, 3, 1, 1)
        self.pool4 = nn.MaxPool2d(2, 2)
        self.conv11 = Conv(512, 512, 3, 1, 1)# 512
